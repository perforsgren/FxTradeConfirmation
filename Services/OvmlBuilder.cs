...
{
    private const string Tag = "OvmlBuilder";
        ...
        string input, CancellationToken ct = default)
    {
         ...
         || ex is IOException)               // prompt file read failure
        {
            ...
            FileLogger.Instance?.Error(Tag, "TryParseAsync failed — returning empty result", ex);
    ...
    private async Task<string> CallApiAsync(string userInput, string systemPrompt, CancellationToken ct)
    {
        ...
        var userText = $"(Today is {today}.) {userInput}";
    ...
    {
        // Strip any slash the AI may have included (e.g. "EUR/SEK" → "EURSEK")
        if (!string.IsNullOrWhiteSpace(po.Pair))
            po.Pair = po.Pair.Replace("/", string.Empty).Trim();
