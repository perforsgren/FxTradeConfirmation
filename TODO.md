...
**Tasks:**
- [ ] In `ClipboardWatcher` / `MainViewModel.RunClipboardFlowAsync()`, snapshot
      the raw clipboard content (text) immediately before any parse or paste
      operation begins.
- [ ] Pass the snapshot into `ClipboardCaptureDialog` (or keep it in the ViewModel)
      so it can be restored on dialog close.
- [ ] On `ClipboardCaptureDialog.OnClosed` (for every result — Reject, PopulateUi,
      or Both):
...
- [ ] Ensure this also runs when `SendToBloombergAsync` completes (after the OVML
      is pasted, restore the original text).
- [ ] Handle edge cases: clipboard content may have changed to something else
