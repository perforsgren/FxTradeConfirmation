...
public static class NotionalParser
{
    ...
    public static int? CountInputDecimals(string? input)
    {
        ...
        if (commaCount == 1 && dotCount == 0)
        {
            // Mirror Parse exactly: 3 digits after comma = thousands separator, not decimal
            int commaIndex = normalized.IndexOf(',');
            int digitsAfterComma = normalized.Length - commaIndex - 1;

            if (digitsAfterComma == 3)
                normalized = normalized.Replace(",", "");   // thousands → strip, no decimal
            else
                normalized = normalized.Replace(',', '.');  // decimal → replace
        }
        else if (commaCount > 0)
        {
