...
public class ReferenceData
{
    ...
    private IReadOnlyList<string> _currencyPairs = [];

    /// <summary>Known FX pairs (e.g. "EURSEK"). Setting this also rebuilds <see cref="CurrencyPairSet"/>.</summary>
    public IReadOnlyList<string> CurrencyPairs
    {
        get => _currencyPairs;
        init
        {
            _currencyPairs = value;
            CurrencyPairSet = new HashSet<string>(value, StringComparer.OrdinalIgnoreCase);
        }
    }

    /// <summary>O(1) lookup counterpart to <see cref="CurrencyPairs"/>. Built automatically on init.</summary>
    public HashSet<string> CurrencyPairSet { get; private set; } = [];
